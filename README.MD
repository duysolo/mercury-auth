...
```shell
npm install --save @mercury-labs/hashing @mercury-labs/auth
```
Learn more about the package `@mercury-labs/hashing` [@mercury-labs/hashing](https://www.npmjs.com/package/@mercury-labs/hashing)
...
export class CmsAuthRepository implements AuthRepository {
    public constructor(protected readonly hasher: PasswordHasherService) {
    }
...
export class Pbkdf2PasswordHasherService implements PasswordHasherService<IPbkdf2Hash> {
    public hash(password: string): Promise<IPbkdf2Hash> {
    ...
    public compare(password: string, hashedPassword: IPbkdf2Hash): Promise<boolean> {
...
AuthModule.forRootAsync({
    ...
    providers: [
        ...,
        {
            ...
            useClass: Pbkdf2PasswordHasherService,
        },
    ],
})
```
